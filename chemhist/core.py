...
def dfmake(arg0,arg3,arg4,arg5,arg6,aparam2,aparam3,df,df_csv): #----------------------------------------------------------------------------------                                                                        
    ...
    for j in data_list:
        field =j.str.split(expand=True)   
        orgx.append(field.iloc[0, int(clmx)-1])                                                                                                      
        if not clmy == 0:
            orgy.append(field.iloc[0, int(clmy)-1])
        else:
            orgy.append(1)
...
def get_descriptor(reference, algebricdesc=True, matrixdesc=True):
    ...
    #df_csv=pd.read_csv('./chemhist/data/input_data.csv')
    #elem_csv=pd.read_csv('DefElem.csv')
    #elem_csv=df_csv.loc[0:101,'AN':'spdf']
    #df_csv=df_csv.loc[0:5,'AN_':'PG_his']
    #print(elem_csv)
    
    # モジュールのルート（core.pyがあるフォルダの絶対パス）
    MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(MODULE_DIR, "data")

    # ファイルパスを安全に構築
    def_elem_path = os.path.join(DATA_DIR, "def_elem_property.csv")
    def_hist_path = os.path.join(DATA_DIR, "def_hist_settings.csv")

    # データ読込
    elem_csv = pd.read_csv(def_elem_path)
    df_csv = pd.read_csv(def_hist_path)
    ...
    for FN in filename:    
        ...
        if aparam3 == 'T':
            ...
            while i<=len(x[0])-1:      
                ...
                while j <= len(x[0])-1:     
                    ...
                    ENatom2=x[2][j]      
                    inty2=x[0][j]/x[3]  
                    #print(readdefelem(ENatom1,elem_csv)[FN][ENatom1])
                    if readdefelem(ENatom1,elem_csv)[FN][ENatom1] != 'nodata' and readdefelem(ENatom2,elem_csv)[FN][ENatom2] != 'nodata':
        ...
        list_reference = reference.split()
        ...
        z[6]=z[6].set_axis(new_col, axis='columns')
        df_dict[FN]=z[6]
    ...
    def to_numpy_dict(df_dict):
        """Convert dict of DataFrames to dict of numpy arrays and label lists."""
        np_dict = {}
        labels_dict = {}
    ...
    df_hiki_dict_np, hiki_labels = to_numpy_dict(df_hiki_dict)

    # DF to Numpy
    def safe_np(df):
        return df.to_numpy(dtype=float) if df is not None and not df.empty else None
    ...
    def extend_from_dict(np_dict, label_dict):
        for key in np_dict.keys():
            if np_dict[key].size > 0:
                arrays.append(np_dict[key].ravel())
                labels.extend([f"{col}" for col in label_dict[key]])

    extend_from_dict(df_dict_np, labels_dict)
    extend_from_dict(df_kake_dict_np, kake_labels)
    extend_from_dict(df_hiki_dict_np, hiki_labels)

    for arr, df_label, name in [
        (k_np, k_df.columns if k_np is not None else [], "k"),
        (h_np, h_df.columns if h_np is not None else [], "h"),
        (l_np, l_df.columns if l_np is not None else [], "l"),
    ]:
        if arr is not None:
            arrays.append(arr.ravel())
            #labels.extend([f"{name}_{col}" for col in df_label])
            labels.extend([f"{col}" for col in df_label])
...
if __name__ == "__main__":
    ...
    args = sys.argv[1:]
    if len(args) == 0:
        print("Usage: python core.py <composition1> [composition2 ...] [--matrix] [outcsv=filename.csv]")
        print("Example: python core.py Li0.5Mn0.5O2 Na3PS4 outcsv=example.csv --matrix")
        sys.exit(0)

    #default paramters
    out_csv = "comphist.csv"
    matrix_flag = False
    comp_list = []

    for a in args:
        if a.startswith("outcsv="):
            out_csv = a.split("=", 1)[1]
        elif a == "--matrix":
            matrix_flag = True
        else:
    ...
    if len(comp_list) == 0:
        print("No composition formula(s) provided. Exiting.")
        sys.exit(0)
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for comp in comp_list:
            ...
            try:
                vec, labels = get_descriptor(
                    comp,
                    algebricdesc=True,
                    matrixdesc=matrix_flag
                )

                if not header_written:
                    writer.writerow(["formula"] + labels)
                    header_written = True

                writer.writerow([comp] + list(vec))
